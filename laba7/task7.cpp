...
int main(int argc, char const *argv[]) {
    ...
    #pragma acc data copyin(maxIndex, previousMatrixPtr[0:matrixSize*matrixSize], currentMatrixPtr[0:matrixSize*matrixSize], matrixSize, alpha)
    {
        while (iteration < maxIterations && error > accuracy) {
            #pragma acc parallel loop independent collapse(2) present(currentMatrixPtr, previousMatrixPtr)
            ...
            if ((iteration + 1) % 10000 == 0) {
                ...
                {
                    status = cublasDaxpy(cublasHandle, matrixSize * matrixSize, &alpha, currentMatrixPtr, 1, previousMatrixPtr, 1); //previousMatrix = previousMatrix - currentMatrix
                    if (status != CUBLAS_STATUS_SUCCESS) {
                   ...
                   status = cublasIdamax(cublasHandle, matrixSize * matrixSize, previousMatrixPtr, 1, &maxIndex); //индекс максимального по модулю элемента в результирующей матрице
                    if (status != CUBLAS_STATUS_SUCCESS) {
                ...
                #pragma acc update self(previousMatrixPtr[maxIndex - 1]) //с GPU на CPU
                error = fabs(previousMatrixPtr[maxIndex - 1]);
                std::cout << "Итерация: " << iteration + 1 << " ошибка: " << error << std::endl;

                #pragma acc host_data use_device(currentMatrixPtr, previousMatrixPtr)
                {
                    status = cublasDcopy(cublasHandle, matrixSize * matrixSize, currentMatrixPtr, 1, previousMatrixPtr, 1); //previousMatrix = currentMatrix
                    if (status != CUBLAS_STATUS_SUCCESS) {
